...
export function TransactionList({ transactions, receipts, onUpdateTransaction, onManualMatch }: TransactionListProps) {
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<ProcessedBankTransaction | null>(null);
  ...
  const findReceiptForTx = (tx: ProcessedBankTransaction | null) => {
    if (!tx || !tx.matchedReceiptId) return undefined;
    return receipts.find(r => r.id === tx.matchedReceiptId);
  }

  const renderRow = (tx: ProcessedBankTransaction) => {
    const isClickable = tx.matchStatus === 'matched' || tx.matchStatus === 'manual';
    return (
        <TableRow 
            key={tx.id}
            onClick={() => isClickable && setSelectedTransaction(tx)}
  ...
  return (
    <>
        ...
        <CardHeader>
            ...
            <CardDescription>Review, categorize, and match transactions from your statement. Click a matched row for details.</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <Dialog open={!!selectedTransaction} onOpenChange={(isOpen) => !isOpen && setSelectedTransaction(null)}>
            <DialogContent className="max-w-4xl p-0">
                {selectedTransaction && findReceiptForTx(selectedTransaction) && (
                    <ExpenseDetails 
                        expense={findReceiptForTx(selectedTransaction)!} 
